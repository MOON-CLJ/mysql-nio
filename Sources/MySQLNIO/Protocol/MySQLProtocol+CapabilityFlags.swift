extension MySQLProtocol {
    ...
    public struct CapabilityFlags: OptionSet, CustomStringConvertible {
        ...
        /// Client: Switches to compressed protocol after successful authentication.
        ...
        public var description: String {
            CapabilityFlags.all.filter(self.contains(_:)).map(\.name).joined(separator: ", ")
        }
        
        /// MySQL specific flags
        var general: UInt32 {
            UInt32(self.rawValue & 0xFFFFFFFF)
        }
        ...
        var mariaDBSpecific: UInt32 {
            get { UInt32(self.rawValue >> 32) }
            set { self.rawValue |= UInt64(newValue) << 32 }
        }
